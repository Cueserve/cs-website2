...
export const RollingButton = forwardRef<HTMLAnchorElement, RollingButtonProps>(
  ({ text, href = "#contact", className = "", variant = "secondary", ...rest }, ref) => {
    ...
    const pillClass = isPrimary
      ? "bg-[#2d8cff] hover:bg-[#1a75e8] text-white border border-[#2d8cff]"
      : "bg-white hover:bg-slate-50 text-slate-900 border border-slate-200";

    const arrowClass = isPrimary
      ? "bg-white text-[#2d8cff] group-hover:bg-slate-50"
      : "bg-[#2d8cff] text-white group-hover:bg-[#1a75e8]";

    return (
      <Link
        ref={ref}
        href={href}
        className={`group relative inline-flex items-center gap-2 md:gap-3 pl-4 md:pl-6 pr-1 md:pr-1.5 py-1 md:py-1.5 rounded-full text-[13px] md:text-[16px] transition-all duration-[180ms] ease-[cubic-bezier(0.16,1,0.3,1)] shadow-md hover:shadow-lg shrink-0 ${pillClass} ${className}`}
        {...rest}
      >
        {/* Staggered Letter-by-Letter Rolling Text Animation */}
        <span className="inline-flex items-center overflow-hidden h-5 md:h-7 leading-5 md:leading-7 font-sans">
          {text.split("").map((char, index) => (
            <span key={index} className="relative inline-block overflow-hidden h-5 md:h-7 leading-5 md:leading-7">
              <span
                className="inline-block transition-transform duration-[180ms] ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:-translate-y-full"
                style={{ transitionDelay: `${index * 8}ms` }}
              >
                {char === " " ? "\u00A0" : char}
              </span>
              <span
                className="absolute inset-0 inline-block translate-y-full transition-transform duration-[180ms] ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:translate-y-0"
                style={{ transitionDelay: `${index * 8}ms` }}
              >
                {char === " " ? "\u00A0" : char}
              </span>
